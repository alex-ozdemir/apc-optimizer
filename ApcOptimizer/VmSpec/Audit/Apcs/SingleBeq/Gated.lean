...
/-! **The pipeline's output: out of the checker's reach, and no step layout.** The last pass mints
    a fresh `is_valid` column, scales every multiplicity by it, and replaces the `1` on the right
    of the comparison constraint by it too — which both defeats the constant-folding tier and
    admits an all-zero padding row. `GatedPinned.lean` pins `is_valid` and recovers everything. -/
...
/-- **The checker does not reach the pipeline's final output.** `gated`'s multiplicities are
    ...
    on a circuit whose clauses are in fact true. -/
theorem gated_checkMultiplicities_fails :
    checkMultiplicities apcRules.isStateful gated = false := by decide

/-- **The padding row.** Every column zero satisfies all seven of this stage's constraints — the
...
/-- **The padding row has no step layout**, exactly as for the other two APCs: the all-zero
    ...
    `StepLayout` asks for a bridge receive netting `-1` on *every* algebraically-satisfying
    assignment. The unoptimized APC has no such row (`unopt_zero_not_satisfiesAlgebraic`), so the
    gating pass is what breaks legality. -/
