...
/-! **`TwoLoads`'s `opt` against `Circuit.legalGuestOF`**, by one `decide`.
...
theorem optOFCheck :
    ofCheckAll layoutVars optPinRules openVmMemBusId openVmTimestampBound 8
      opt.busInteractions recipes optPartnerList = true := by decide

theorem opt_hasStepLayoutOF {maxWindow : ℕ} (hw : 8 < maxWindow) :
...
theorem opt_legalGuestOF {maxWindow maxInteractions : ℕ} (hw : 8 < maxWindow)
